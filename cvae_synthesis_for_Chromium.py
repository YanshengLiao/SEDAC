...
# Text representation
chromium = pd.read_csv("data/Chromium.csv")
chromium_chunk = pd.read_csv("data/Chromium.csv", iterator=True, chunksize=1000)
...
br = torch.empty((0,768))
for chunk in chromium_chunk:
    features = feature_extraction(list(chunk["sentences"]))
    a = torch.mean(torch.tensor(features[0][0]),dim = 0).unsqueeze(0)
    for i in range(len(features)-1):
        a = torch.cat([a,torch.mean(torch.tensor(features[i+1][0]),dim = 0).unsqueeze(0)],dim=0)
    br = torch.cat([br,a])

labels = pd.DataFrame(chromium["Security"])
